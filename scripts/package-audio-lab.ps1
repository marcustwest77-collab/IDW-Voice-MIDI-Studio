...
python -m pip install pyinstaller==6.16.0 Pillow==11.3.0
...
try {
    ...
    $env:PYTHONPATH = ''
    $process = Start-Process -FilePath $exe -ArgumentList @('--self-test', '--test-report', ('"' + $report + '"'), '--preview-path', ('"' + $preview + '"')) -WorkingDirectory $isolated -PassThru

param([string]$JucePath = "", [string]$BuildDirectory = "build-win-4.1")
...
$release = Join-Path (Get-Location) ('IDW-V4.1-Windows-' + (Get-Date -Format 'yyyyMMdd-HHmmss'))
...
'@echo off', 'start "IDW V4.1" "%~dp0Windows\Standalone\IDW Voice MIDI Studio.exe"' | Set-Content "$release\Run-IDW.cmd"
